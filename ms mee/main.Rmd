...
The *ecophylo* package essentially articulates itself around the *simulate* function. This function implements the above-mentioned simulation algorithm and allows users to simulate a phylogeny in Newick format given the desired parameter combinations accounting for the demographic history of $J_m$. Each demographic event accounting for the history of $J_m$ occurs at a specific time which users must supply as a list in the order in which they occur (backwards in time). 

In the following example, an assemblage of species and their phylogenetic relationships, is simulated assuming that $J_m$ has fluctuated in the past over 3 predefined periods. Users can input the desired time periods (in generations before present) by specifying the *changetime* parameter, yet the *timeframes* function can be used to determine the periods while accounting for loss of resolution in more ancient periods (Boitard et.al 2016 Plos Genetics). Changes in sizes can be instantaneous (by providing a list of past sizes in *past_sizes*). Users can also model assemblages with exponential growth or contraction by providing a list of growth rates $\alpha$ for each desired time period. The size of the assemblage at a given time $t_n$  in the past (provided in the *changetime* parameter) is computed as $S_n = S_{n-1}.e^{-\alpha_{n}(t_{n} - t_{n-1})}$. 

```{python}
import ecophylo

tree = ecophylo.simulate(samples = 10, com_size= [[500,1000,5000]], mu = 0.01, changetime=[[0,700,10000]], seed= 42)
```
...
n <- list(as.integer(10)) # the number of simulated individuals
comsize <- list(list(500, 1000, 5000)) # the size of the assemblage in the past, the first element is the current assemblage size
mu <- 0.01 # the point mutation rate

#epochs <- ecophylo$timeframes(I= as.integer(2), T= as.integer(10000), a=0.3) # the dates at which the assemblage has changed sizes in the past

epochs <- list(list(0, 700, 10000))
...
samples = [10]
com_size = [[500.0, 1000.0, 5000.0]]
mu = 0.01
changetime = [[0.0, 700.0, 10000.0]]
seed = 42
init_rates = [[0]]
mrca = None
migr = 1
...
import ecophylo
ecophylo.simulate(samples=samples, com_size=com_size, mu=mu, changetime=changetime )
