...
Jm <- as.integer(1e5) # the size of the metacommunity (sensu Hubbell, 2001)
mu <- as.numeric(0.03) # the point mutation rate

tree <- ecophylo$simulate(n, Jm, mu, seed = as.integer(42))
abund <- ecophylo$getAbund(tree, n)

#TODO : Find a way to compute PD, MPD, MNTD from 'tree' object directly in R 
```
...
tree = ecophylo.simulate(10, 1e5, 0.03, seed = 42)
print(tree)
```

### Simulating past community-wide demographic fluctuations

However, Jm can only be approximately considered as the harmonic mean of the per-generation Jm dynamics. Thus by allowing Jm  to vary at specific ages of the past we can relax this equilibrium assumption and account for the discordance of demographic histories between distinct metacommunities.
...
### Simulating sub-populations inked by vicariance and migration events
