...
```{r}
pastsizes <- list(as.integer(1000)) # the size of the metacommunity in the past 
changetime <- list(as.integer(100)) # the time (in generations) at which metacommunity size has changed
...
tr = ecophylo.simulate(sample_size,com_size,mu,past_sizes=past_sizes, changetime=chagetime, verbose = False, seed= 42)
...
```{r}
# Faith's phylogenetic diversity (PD)
pd(matrix(abund1, nrow =1, dimnames = list("", phylo1$tip.label)), phylo)[[1]]

# Mean pairwise distance (MPD)
mpd(matrix(abund1, nrow =1, dimnames = list("", phylo1$tip.label)), cophenetic(phylo1))

# Mean nearest taxon distance (MNTD)
mntd(matrix(abund1, nrow =1, dimnames = list("", phylo1$tip.label)), cophenetic(phylo1)) 
```
