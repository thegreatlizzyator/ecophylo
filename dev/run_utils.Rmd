...
if npop > 1:
  ...
  if npop > 1:
    ...
    random_seed= seed,
    population_configurations = popconfig, # ici joue le même role que sample_size
    migration_matrix = migration,
    demographic_events = demography)
  else: 
...
# besoin de calculer les init_rates a partir des premiers elements de past sizes.
# et les comparer aux premiers gr si donnés.

# past_values = [[0.2, 0.1, -0.09, 0.009], [0.42, -0.4, 0.07, 0.42,0.01]]
# sizevalues = False

past_rates = eco.mergesizes2rates(past_values, changetime, init_size, False)
past_rates
# [[  10,     15,     20,    21,     30,     40,     60,     70], 
# [0.069, -0.014, -0.014,  0.04,   0.04, -0.009, -0.009, -0.009], 
# [-0.11,  0.046,  0.044, 0.044, -0.015, -0.015, -0.015,  0.006]]

population_changes(paste_rates)

population_configurations(init_size, paste_sizes, init_rates, sample)

# soit besoin de fournir les init_rates soit les trouver a partir de past_sizes.
...
if stable_pop:
  pc = [msprime.PopulationConfiguration(sample_size = s, initial_size = i) for s, i in zip(samples, init_sizes)]
  
  pastdemo = []
  for i in range(len(changetime)):
    pastdemo = pastdemo +  [msprime.PopulationParametersChange(time=t, initial_size =s, population_id= i) for t, s in zip(changetime[i], past_sizes[i])]
  
elif rates == None :
  rates = eco.mergesizes2rates(past_sizes, changetime, init_size, True)
  sizes = eco.mergesizes2rates(past_sizes, changetime, init_size, False)
  rate_time = rates[0]
  pc = []
  for i in range(len(changetime)):
    pc = pc + [msprime.PopulationConfiguration(sample_size = samples[i], growth_rate= rates[i+1][0],  initial_size = init_sizes[i] )]
    rates[i + 1].remove(rates[i+1][0])
    rates[i+1].append(rates[i+1][-1])
  
  # print(rates)
    # [[10, 15, 20, 30, 60], 
    # [-0.056, -0.014, -0.014, 0.028, 0.028],
    # [-0.161, 0.046, 0.015, 0.015, 0.015]]
  pastdemo = []
  # print(rate_time)
  for i in range(len(changetime)): # TODO : need to sort this by time now....
    for ii in range(len(rate_time)):
      # print(i, ii, len(rate_time), len(sizes[i]), len(rates[i+1]))
      # print(rate_time[ii], sizes[i][ii],rates[i+1][ii] )
      pastdemo = pastdemo +  [msprime.PopulationParametersChange(
        time=rate_time[ii],
        initial_size = sizes[i][ii],
        growth_rate=rates[i+1][ii],
        population_id= i)]
