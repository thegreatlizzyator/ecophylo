...
```{r, eval=TRUE}
if(!"reticulate"%in%installed.packages()[,1]) install.packages('reticulate') # answer yes
library(reticulate)
# python dependencies
conda_install('r-reticulate', c('tskit', 'msprime','ete3','pandas',"Numpy"))
py_install("ecophylo", envname = "r-reticulate", method = "conda", pip = TRUE)
```
...
```{r, eval=F}
# chunk to check if Maxime don't break the function !
pastsizes <- list(as.integer(1000)) # the size of the metacommunity in the past 
changetime <- list(100) # the time (in generations) at which metacommunity size has changed


cat(py_capture_output( # capture the verbose
  tree1b <- ecophylo$simulate_dolly(n, Jm, mu,
                             past_sizes = pastsizes,
                             changetime = changetime,
                             seed = 42, verbose = TRUE),
  type = "stderr")) # verbose is eported as an error
...
# TO DO: print demmography debuger here 
```

```{python eval=FALSE, include=FALSE}
### Why import again here?
import ecophylo 
